...
**npm scripts**: `bun run dev` (watch mode), `bun run build`, `bun run start` (prod), `bun run lint`, `bun run format`, `bun run db:generate` / `db:migrate` / `db:push`, `bun run db:sync` (assemble schema from enabled modules + push + sync menu/permission catalog), `bun run db:seed` (default roles + admin user), `bun audit`  
