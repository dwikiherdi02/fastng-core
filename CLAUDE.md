...
- **Module manifest**: A module that is a sidebar menu (or exposes permissions) declares `src/modules/{name}/{name}.manifest.ts` exporting `manifest: ModuleManifest` (`menu` metadata or `menu: false`, plus `permissions[]`). Synced to the DB catalog bidirectionally by `yarn db:sync`. See `tutorial/19`.
